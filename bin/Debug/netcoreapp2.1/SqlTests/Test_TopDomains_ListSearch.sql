...
SELECT
           @PLMembership = COUNT(DISTINCT tblProfileContents.profile_id)
FROM
           tblProfileListResults
           INNER JOIN
                      tblProfiles
                      ON
                                 tblProfileListResults.profile_id = tblProfiles.profile_id
           INNER JOIN
                      tblProfileContents
                      ON
                                 tblProfiles.profile_id = tblProfileContents.profile_id
WHERE
           profileList_id                         = 420
           AND [tblProfileContents].authoredTime >= '20190122 17:46:06.000'
           AND [tblProfileContents].authoredTime <= '20190522 17:46:06.000'
...
WHERE
           ...
           AND
           (
                      [tblProfileContents].[authoredTime] >= '20190122 17:46:06.000'
           )
           AND
           (
                      [tblProfileContents].[authoredTime] <= '20190522 17:46:06.000'
           )
           AND
           (
                      tblProfiles.profile_id IN
                      (
                             SELECT
                                    profile_id
                             FROM
